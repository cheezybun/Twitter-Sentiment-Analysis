...
def setup_model():
    ...
    nlp = pipeline("sentiment-analysis", model='NLP-Model')
    return nlp
...
def main():
    ...
    with col1:
        ...
        st.subheader("Made by Nikhil Kumar")
    with col2:
        st.title("")
    menu = ["Home","About"]
    ...
    if choice == "Home":
        with col1:
            ...
            opt = st.selectbox('Language',pd.Series(['English','Italian']))
            ...
            with st.form(key='nlpForm'):
                raw_text = st.text_area("Enter your Keyword below:")
        ...
        st.subheader("About")
        st.write("This was made in order to have an idea about Twitter Sentiment Analysis. The program scrapes twitter for tweets.\
             The tweets are then fed into a model for sentiment analysis which is then used \
            to display Sentiment and general word cloud.")
